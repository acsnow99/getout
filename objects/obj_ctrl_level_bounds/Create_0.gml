...
function initialize_level_bounds() {
	...
	for (var i = 0; i < instance_number(obj_level_bound); i++;) {
		...
		global.level_bounds[i, LEVEL_BOUND_PROPS.X_RIGHT] = o.x + sprite_get_width(spr_level_bound);
		global.level_bounds[i, LEVEL_BOUND_PROPS.Y_BOTTOM] = o.y + sprite_get_height(spr_level_bound);
	}
...
level_seek = function(_focusx, _focusy) {
	...
	for (var i = 0; i < instance_number(obj_level_bound); i++) {
		...
		var _max_x = global.level_bounds[i, LEVEL_BOUND_PROPS.Y_TOP];
		var _min_y = global.level_bounds[i, LEVEL_BOUND_PROPS.X_RIGHT];
